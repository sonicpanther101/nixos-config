...
export default ({ monitor }: { monitor: number }) => (
  <window vexpand name={`notifications${monitor}`} className={"notifications-window"} widthRequest={450} anchor={Astal.WindowAnchor.TOP | Astal.WindowAnchor.RIGHT} hexpand={true} layer={Astal.Layer.OVERLAY} application={App}>
    <box vexpand vertical className={"notifications"}>
      {notifications}
    </box>
  </window>
);